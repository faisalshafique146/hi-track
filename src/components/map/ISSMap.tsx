...
export default function ISSMap() {
    ...
    useEffect(() => {
        if (position) {
            const newCoord: [number, number] = [position.latitude, position.longitude];
            setTrail((prev) => {
    ...
    if (isError || !position) {
        return <div className="w-full h-[400px] flex items-center justify-center bg-muted text-muted-foreground rounded-xl">Error loading map data</div>;
    }
